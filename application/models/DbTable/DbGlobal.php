...
class Application_Model_DbTable_DbGlobal extends Zend_Db_Table_Abstract
{
	...
	public function getLoanNumberByBranch($type){
		$db = $this->getAdapter();
		if($type==1){
			$sql="SELECT m.`loan_number` AS id,CONCAT((SELECT name_en FROM `ln_client` WHERE client_id = m.client_id LIMIT 1),'-',m.`loan_number`) AS `name`,g.`branch_id` FROM `ln_loan_member` AS m,`ln_loan_group` AS g WHERE m.`group_id`= g.`g_id` AND m.`is_completed`=0 AND g.`loan_type`=1 AND m.status=1 AND g.status=1 AND m.is_reschedule!=1 ";
...
//    }
   ...
   if($type!=null){
   	...
   	$sql = " SELECT client_id,name_en,client_number,
   				(SELECT `ln_village`.`village_name` FROM `ln_village` WHERE (`ln_village`.`vill_id` = `ln_client`.`village_id`) limit 1) AS `village_name`,
